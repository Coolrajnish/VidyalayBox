...
public interface StudentRepo extends GenericRepo<StudentEntity, Long> {
    ...
    @Query(value = "SELECT u.first_name, u.last_name,u.role, u.gender, u.mobile_number,u.birth_date, p.admission_date,c.class_name, sc.section_name FROM student p JOIN users u on u.id = p.user_id JOIN parent sp on sp.id = p.parent_id JOIN class c on c.id = p.class_id JOIN section sc on sc.id = c.section_id WHERE u.org_uniq_id =:orgId  AND (u.first_name IS NULL OR LOWER(u.first_name) LIKE LOWER(CONCAT(:searchText, '%'))) OR (u.lst_name IS NULL OR LOWER(u.last_name) LIKE LOWER(CONCAT( :searchText, '%'))) OR (u.email IS NULL OR LOWER(u.email) LIKE LOWER(CONCAT( '%',:searchText, '%'))) OR (u.mobile_number IS NULL OR LOWER(u.mobile_number) LIKE LOWER(CONCAT( :searchText, '%')))", nativeQuery = true)
