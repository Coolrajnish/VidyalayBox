...
import com.ms.shared.util.util.rest.GlobalExceptionHandler;
import com.ms.shared.util.util.rest.ResourceNotFoundException;
...
public class AuthController {
    ...
    public AuthController(AuthenticationManager authenticationManager, JwtTokenProvider jwtTokenProvider, IOrgClientService iOrgClientService, IUserService iUserService, EmailService emailService, EmailService emailService1, UserServiceImpl userService) {
    ...
    public ResponseEntity<APiResponse<GenericDTO>> registerOrg(@Valid @RequestBody OrgSignupRequestDTO orgSignupRequestDTO) {
        ...
        try {
            ...
            return ResponseEntity.ok(new APiResponse<>(
                    "error",
                    "Organization signed up failed " + e.getLocalizedMessage(),
                    null,
                    null
            ));
    ...
    public ResponseEntity<APiResponse<Object>> verifyUser(@RequestParam String token) {
        ...
        APiResponse<Object> ApiResponse = new APiResponse<>(
                ...
                isVerified ? "User verified successfully" : "User verification fails",
                null,
                null
        );
    ...
    public ResponseEntity<APiResponse<Object>> resetPassword(@RequestParam String token, @RequestParam String newPassword) {
        if (_userService.validateResetToken(token)) {
            ...
            return resp.getStatus().contains("success") ? ResponseEntity.ok(resp) : ResponseEntity.status(HttpStatus.BAD_REQUEST).body(resp);
        } else {
    ...
    @PostMapping("/signin")
    public ResponseEntity< APiResponse<LoginResponseDTO>> authenticateUser(@Valid @RequestBody LoginRequestDTO loginRequest) throws FatalException {
        try {
            ...
            if (userDetails.isEnabled()) { // Check whether User is active or not
                ...
                return ResponseEntity.ok(
                        new APiResponse<>(
                                ...
                                new LoginResponseDTO(jwtToken, userDetails.getId(),userDetails.getOrgId(), roles, userDetails.isEnabled(),
        ...
        return ResponseEntity.badRequest().body( new APiResponse<>(
                "error",
                "Signed-in failed, User has been deactivated/locked !!",
                null,
                null
        ));
    }
